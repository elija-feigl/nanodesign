...
def main():
    ...
    for vhelix in vhelix_list:
        ...
        for domain_id in domain_list:
            for domain in domains_obj_list:
                ...
                if domain_id == id:
                    ...
                    print(
                        "id: %4d  nbases: %3d  strand: %3d  connected to domain: %4d"
                        % (domain_id, num_bases, strand_id, connected_domain)
                    )
        ...
        for connection in helix_connectivity:
            if connection:
                num_conn += 1
        print(
            ">>> number of possible staple crossovers: %d"
            % num_possible_staple_crossovers
        )
        print(
            ">>> number of possible scaffold crossovers: %d"
            % num_possible_scaffold_crossovers
        )
        ...
        for connection in helix_connectivity:
            ...
            print(
                "helix id: %d  num: %d  dir: (%g %g %g)  angle: %g"
                % (helix_id, helix_num, dir[0], dir[1], dir[2], angle)
            )
            if not crossovers:
            ...
            for crossover in crossovers:
                ...
                print(
                    "    %d: vhelix_base_index: %d  strand1_ID: %d  strand1_bindex: %d"
                    % (n, vhelix_base_index, strand1_ID, strand1_bindex)
                )
                if strand2_ID:
                    print(
                        "       strand2_ID: %d  strand2_bindex: %d"
                        % (strand2_ID, strand2_bindex)
                    )
                n += 1
    ...
    for strand in strand_list:
        ...
        print("")
        print(
            ">>> strand %s: scaffold: %s nbases: %d nvhelix: %d "
            % (id, is_scaffold, len(bases), len(vhelix_list))
        )
        print(
            "              number of domains: %d  domains: %s"
            % (len(strand_domain_list), str(strand_domain_list))
        )
        print("              color: %g %g %g" % (color[0], color[1], color[2]))
        print("              bases: ")
        print(
            "              total number of domain bases: %d" % num_strand_domain_bases
        )
        n = 0
        for base in bases:
            id = base["id"]
            ...
            seq = base["sequence"]
            print(
                "              %d: id %4d  coord (%6g, %6g, %6g) seq %s"
                % (n, id, coord[0], coord[1], coord[2], seq)
            )
            n += 1
    ...
    for domain in domains_list:
        ...
        print("")
        print(
            ">>> domain %s: strand: %d  connected to strand: %d  connected to domain: %d "
            % (id, strand_id, connected_strand, connected_domain)
        )
        print("              nbases: %d  bases %s " %
              (len(base_list), str(base_list)))
        print(
            "              start_pos: (%g %g %g) end_pos (%g %g %g) "
            % (
                start_pos[0],
                start_pos[1],
                start_pos[2],
        ...
        print(
            "              start_base_index: %d  end_base_index: %d "
            % (start_base_index, end_base_index)
        )
        print("              color: (%g, %g, %g) " %
