...
class Domain(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error(
                "Domain data is missing the JSON %s field: %s"
                % (str(e), str(json_data))
            )
            self.bad_data = True
    ...
    def compare(self, domain):
        ...
        if not compare_lists(
            self.BASES, self.base_list, domain.base_list, check_order=True
        ):
            self.num_errors += 1
        ...
        if not compare_float_lists(self.orientation, domain.orientation):
            logger.error(
                "'%s' values are not equal: %s != %s"
                % (self.ORIENTATION, self.orientation, domain.orientation)
            )
            self.num_errors += 1
        if not compare_float_lists(self.start_pos, domain.start_pos):
            logger.error(
                "'%s' values are not equal: %s != %s"
                % (self.START_POS, self.start_pos, domain.start_pos)
            )
            self.num_errors += 1
        if not compare_float_lists(self.end_pos, domain.end_pos):
            logger.error(
                "'%s' values are not equal: %s != %s"
                % (self.END_POS, self.end_pos, domain.end_pos)
            )
            self.num_errors += 1
...
class Base(object):
    ...
    def __init__(self, json_data, id=0):
        ...
        try:
            ...
            logger.error(
                "Base data is missing the JSON %s field: %s" % (
                    str(e), str(json_data))
            )
            self.bad_data = True
    ...
    def compare(self, base):
        ...
        if not compare_float_lists(self.coord, base.coord):
            logger.error(
                "Base number %d '%s' values are not equal: %s != %s"
                % (self.id, self.COORDS, self.coord, base.coord)
            )
            self.num_errors += 1
...
class Strand(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error(
                "Strand data is missing the JSON %s field: %s"
                % (str(e), str(json_data))
            )
            self.bad_data = True
    ...
    def compare(self, strand):
        ...
        if not compare_lists(
            self.DOMAINS, self.domain_list, strand.domain_list, check_order=True
        ):
            self.num_errors += 1

        # Compare the list of helix IDs.
        if not compare_lists(
            self.VIRTUAL_HELICES, self.helix_list, strand.helix_list, check_order=True
        ):
            self.num_errors += 1
        ...
        if len1 != len2:
            logger.error(
                "'%s' the number of bases are not equal: %d != %d"
                % (self.BASES, len1, len2)
            )
            self.num_errors += 1
            return
        ...
        if id_list:
            logger.error(
                "'%s' IDs are different: %s not found in second file."
                % (self.BASES, id_list)
            )
            self.num_errors += 1
            return
...
class HelixCrossover(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error(
                "Helix crossover data is missing the JSON %s field: %s"
                % (str(e), str(json_data))
            )
            self.bad_data = True
...
class HelixConnectivity(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error(
                "Helix connectivity data is missing the JSON %s field: %s"
                % (str(e), str(json_data))
            )
            self.bad_data = True
    ...
    def compare(self, helix_conn):
        ...
        if not compare_float_lists(self.dir, helix_conn.dir):
            logger.error(
                "'%s' values are not equal: %s != %s"
                % (self.DIRECTION, self.dir, helix_conn.dir)
            )
            self.num_errors += 1
        ...
        if len1 != len2:
            logger.error(
                "'%s' the number of crossovers are not equal: %d != %d"
                % (self.CROSSOVERS, len1, len2)
            )
            self.num_errors += 1
            return
        ...
        for id, crossover in map1.items():
            logger.info("'%s' %s %d " %
                        (self.CROSSOVERS, crossover.BASE_INDEX, id))
...
class Helix(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            self.num_possible_staple_crossovers = json_data[
                self.NUM_POSSIBLE_STAPLE_XOVERS
            ]
            self.num_possible_scaffold_crossovers = json_data[
                self.NUM_POSSIBLE_SCAFFOLD_XOVERS
            ]
            ...
            logger.error(
                "Helix data is missing the JSON %s field: %s" % (
                    str(e), str(json_data))
            )
            self.bad_data = True
    ...
    def compare(self, helix):
        ...
        if not compare_float_lists(self.orientation, helix.orientation):
            logger.error(
                "'%s' values are not equal: %s != %s"
                % (self.ORIENTATION, self.orientation, helix.orientation)
            )
            self.num_errors += 1
        if not compare_float_lists(self.start_pos, helix.start_pos):
            logger.error(
                "'%s' values are not equal: %s != %s"
                % (self.START_POS, self.start_pos, helix.start_pos)
            )
            self.num_errors += 1
        if not compare_float_lists(self.end_pos, helix.end_pos):
            logger.error(
                "'%s' values are not equal: %s != %s"
                % (self.END_POS, self.end_pos, helix.end_pos)
            )
            self.num_errors += 1

        # Compare domain ID lists.
        if not compare_lists(
            "domains", self.domain_ids, helix.domain_ids, check_order=True
        ):
            self.num_errors += 1
    ...
    def compare_hconn(self, helix):
        ...
        if len1 != len2:
            logger.error(
                "'%s' the number of connected helices are not equal: %d != %d"
                % (self.HELIX_CONNECTIVITY, len1, len2)
            )
            self.num_errors += 1
            return
        ...
        if id_list:
            logger.error(
                "'%s' ID/num pairs are different: %s not found in second file."
                % (self.HELIX_CONNECTIVITY, id_list)
            )
            self.num_errors += 1
            return
        ...
        for cindex, conn in conn_map1.items():
            logger.info(
                "'%s' id %d  num %d " % (
                    self.HELIX_CONNECTIVITY, cindex[0], cindex[1])
            )
...
class DnaStructure(object):
    ...
    def parse_json(self, json_data):
        ...
        if self.LATTICE_TYPE not in json_data:
            logger.error(
                "The '%s' field is missing from the JSON file." % self.LATTICE_TYPE
            )
        else:
    ...
    def parse_helix_data(self, json_data):
        ...
        if self.VIRTUAL_HELICES not in json_data:
            logger.error(
                "The '%s' field is missing from the JSON file." % self.VIRTUAL_HELICES
            )
            return
    ...
    def parse_strand_data(self, json_data):
        ...
        if self.STRANDS not in json_data:
            logger.error(
                "The '%s' field is missing from the JSON file." % self.STRANDS)
            return
    ...
    def parse_domain_data(self, json_data):
        ...
        if self.DOMAINS not in json_data:
            logger.error(
                "The '%s' field is missing from the JSON file." % self.DOMAINS)
            return
    ...
    def compare(self, dna_structure):
        ...
        if self.lattice_type != dna_structure.lattice_type:
            logger.error(
                "Lattice types not equal: %s != %s"
                % (self.lattice_type, dna_structure.lattice_type)
            )
            self.num_errors += 1
        ...
        if self.num_errors != 0:
            logger.error(
                "Files are not identical: %d errors detected." % self.num_errors
            )
            return False
        else:
    ...
    def compare_domain(self, dna_structure):
        ...
        if len(self.domain_list) != len(dna_structure.domain_list):
            logger.error(
                "Number of domains are not equal: %d != %d"
                % (len(self.domain_list), len(dna_structure.domain_list))
            )
            self.num_errors += 1
        ...
        if id_list:
            logger.error(
                "Domain IDs are not equal: %s not found in second file domain IDs."
                % id_list
            )
            self.num_errors += 1
            return
    ...
    def compare_strand(self, dna_structure):
        ...
        if len(self.strand_list) != len(dna_structure.strand_list):
            logger.error(
                "Number of strands are not equal: %d != %d"
                % (len(self.strand_list), len(dna_structure.strand_list))
            )
            self.num_errors += 1
        ...
        if id_list:
            logger.error(
                "Strand IDs are not equal: %s not found in second file strand IDs."
                % id_list
            )
            self.num_errors += 1
            return
    ...
    def compare_helix(self, dna_structure):
        ...
        if len(self.helix_list) != len(dna_structure.helix_list):
            logger.error(
                "Number of helices are not equal: %d != %d"
                % (len(self.helix_list), len(dna_structure.helix_list))
            )
            self.num_errors += 1
        ...
        if num_list:
            logger.error(
                "Helix nums are not equal: %s not found in second file helix nums."
                % num_list
            )
            self.num_errors += 1
            return
...
def compare_dicts(dict1, dict2):
    ...
    num_errors = 0
    for attr, value1 in dict1.items():
        value2 = dict2[attr]
        if (type(value1) in (bool, int, float, str)) and (value1 != value2):
            logger.error(
                "'%s' values are not equal: %s != %s" % (
                    attr, str(value1), str(value2))
            )
            num_errors += 1
    return num_errors
...
def compare_lists(name, list1, list2, check_order):
    ...
    if len(list1) != len(list2):
        logger.error(
            "'%s' lists are not the same size: %d != %d"
            % (name, len(list1), len(list2))
        )
        return False
    ...
    if False:
        ...
        if ldiff:
            logger.error(
                "'%s' lists contain different values: %s" % (name, list(ldiff))
            )
            return False

        # Check if the ordering of the lists are the same.
        if check_order and (list1 != list2):
            logger.error(
                "'%s' list values are not in the same order: %s != %s"
                % (name, list1, list2)
            )
            return False
