...
class Domain(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error("Domain data is missing the JSON %s field: %s" %
                         (str(e), str(json_data)))
            self.bad_data = True
    ...
    def compare(self, domain):
        ...
        if not compare_float_lists(self.orientation, domain.orientation):
            logger.error("'%s' values are not equal: %s != %s" % (
...
class Base(object):
    ...
    def __init__(self, json_data, id=0):
        ...
        try:
            ...
            logger.error("Base data is missing the JSON %s field: %s" %
                         (str(e), str(json_data)))
            self.bad_data = True
    ...
    def compare(self, base):
        ...
        if not compare_float_lists(self.coord, base.coord):
            logger.error("Base number %d '%s' values are not equal: %s != %s" % (
...
class Strand(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error("Strand data is missing the JSON %s field: %s" %
                         (str(e), str(json_data)))
            self.bad_data = True
    ...
    def compare(self, strand):
        ...
        if len1 != len2:
            logger.error("'%s' the number of bases are not equal: %d != %d" % (
        ...
        for id in base_map1:
            if id not in base_map2:
                id_list.append(id)
        if id_list:
            logger.error("'%s' IDs are different: %s not found in second file." % (
                self.BASES, id_list))
            self.num_errors += 1
            return
...
class HelixCrossover(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error("Helix crossover data is missing the JSON %s field: %s" % (
                str(e), str(json_data)))
            self.bad_data = True
...
class HelixConnectivity(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error("Helix connectivity data is missing the JSON %s field: %s" % (
                str(e), str(json_data)))
            self.bad_data = True
    ...
    def compare(self, helix_conn):
        ...
        if len1 != len2:
            logger.error("'%s' the number of crossovers are not equal: %d != %d" % (
...
class Helix(object):
    ...
    def __init__(self, json_data):
        ...
        try:
            ...
            logger.error("Helix data is missing the JSON %s field: %s" %
                         (str(e), str(json_data)))
            self.bad_data = True
    ...
    def compare_hconn(self, helix):
        ...
        if len1 != len2:
            logger.error("'%s' the number of connected helices are not equal: %d != %d" % (
        ...
        if id_list:
            logger.error("'%s' ID/num pairs are different: %s not found in second file." %
...
class DnaStructure(object):
    ...
    def parse_helix_data(self, json_data):
        ...
        if self.VIRTUAL_HELICES not in json_data:
            logger.error("The '%s' field is missing from the JSON file." %
                         self.VIRTUAL_HELICES)
            return
    ...
    def compare(self, dna_structure):
        ...
        if self.lattice_type != dna_structure.lattice_type:
            logger.error("Lattice types not equal: %s != %s" %
    ...
    def compare_domain(self, dna_structure):
        ...
        if len(self.domain_list) != len(dna_structure.domain_list):
            logger.error("Number of domains are not equal: %d != %d" % (
        ...
        if id_list:
            logger.error(
                "Domain IDs are not equal: %s not found in second file domain IDs." % id_list)
            self.num_errors += 1
            return
    ...
    def compare_strand(self, dna_structure):
        ...
        if len(self.strand_list) != len(dna_structure.strand_list):
            logger.error("Number of strands are not equal: %d != %d" % (
        ...
        if id_list:
            logger.error(
                "Strand IDs are not equal: %s not found in second file strand IDs." % id_list)
            self.num_errors += 1
            return
    ...
    def compare_helix(self, dna_structure):
        ...
        if len(self.helix_list) != len(dna_structure.helix_list):
            logger.error("Number of helices are not equal: %d != %d" % (
        ...
        if num_list:
            logger.error(
                "Helix nums are not equal: %s not found in second file helix nums." % num_list)
            self.num_errors += 1
            return
...
def compare_lists(name, list1, list2, check_order):
    ...
    """
    if len(list1) != len(list2):
        logger.error("'%s' lists are not the same size: %d != %d" %
    ...
    if (False):
        ...
        if ldiff:
            logger.error("'%s' lists contain different values: %s" %
                         (name, list(ldiff)))
            return False

        # Check if the ordering of the lists are the same.
        if check_order and cmp(list1, list2):
            logger.error("'%s' list values are not in the same order: %s != %s" % (
                name, list1, list2))
            return False
