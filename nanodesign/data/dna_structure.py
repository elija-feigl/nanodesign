...
"""
    ...
    A DNA structure consists of a number of scaffold and staple strands (DNA origami),
    or oligo strands alone, bound together to form a designed geometric shape.
"""
...
class DnaStructure(object):
    ...
    def _add_structure_helices(self, structure_helices):
        ...
        for helix in structure_helices:
            ...
            self.structure_helices_coord_map[(
                helix.lattice_row, helix.lattice_col)] = helix
    ...
    def remove_staples(self, retain_staples):
        ...
        if self._logger.getEffectiveLevel() == logging.DEBUG:
            ...
            for strand in removed_strands:
                self._logger.debug(
                    f"Remove staple strand ID {strand.id} start helix {strand.tour[0].h} p {strand.tour[0].p}")
            ...
            for strand in remaining_strands:
                if not strand.is_scaffold:
                    self._logger.debug(
                        f"Retain staple strand ID {strand.id} start helix {strand.tour[0].h} p {strand.tour[0].p}")
    ...
    def create_base_connectivity_table(self):
        ...
        if self._logger.getEffectiveLevel() == logging.DEBUG:
            self._logger.debug("Base connectivity table size %d" %
    ...
    def generate_maximal_staple_set(self, retain_staples):
        ...
        removed_strands, remaining_strands = self.create_removed_staple_lists(
            retain_staples)
        self._logger.info("Number of retained staples %d" %
                          len(retain_staples))
        ...
        for helix in self.structure_helices_map.values():
            ...
            helix.set_coordinates(axis_coords, axis_frames,
        ...
        self.create_strands()
        self.strands_map = dict()
        self._logger.info("Number of added staples %d" %
    ...
    def remove_helices_bases(self, removed_strands):
        ...
        for strand in removed_strands:
            base = strand.tour[0]
            self._logger.debug("Remove strand ID %d  start h %d  p %d" % (
    ...
    def _compute_domains(self):
        ...
        for strand in self.strands:
            ...
            if (strand.is_scaffold):
                self._logger.debug(
                    "==================== scaffold strand %d ====================" % strand.id)
            else:
                self._logger.debug(
                    "==================== staple strand %d ====================" % strand.id)

            start_base = strand.tour[0]
            end_base = strand.tour[-1]
            self._logger.debug("Strand number of bases: %3d" %
                               len(strand.tour))
            self._logger.debug("Strand start: h: %3d  p: %3d" %
                               (start_base.h, start_base.p))
            self._logger.debug("Strand end: h: %3d  p: %3d" %
            ...
            for i in range(1, len(strand.tour)):
                ...
                if curr_across_sign != across_sign:
                    domain_id = self._add_domain(
                        domain_id, strand, domain_bases, merge_domains, "sign change")
                    domain_bases = []
                    curr_across_sign = across_sign
                ...
                # Check for a crossover between helices for this base.
                    ...
                    if base.h == last_base.h:
                        ...
                        add_curr_base = False
                    domain_id = self._add_domain(
                        domain_id, strand, domain_bases, merge_domains, "base crossover")
                    domain_bases = []
                ...
                # Check the base paired to this base for: crossover or termination.
                    ...
                    # If a strand terminates make sure the current base is in the same strand.
                        ...
                        if last_base.across is not None:
                            ...
                            add_curr_base = False
                        domain_id = self._add_domain(
                            domain_id, strand, domain_bases, merge_domains, "abase start/end")
                        domain_bases = []
        ...
        self._logger.info("Number of domains computed: %d " %
    ...
    def check_domains(self):
        """ Check that the bases in the domains created for a structure are consistent with the bases in the strand
            ...
            The combination of the bases in a list of domains for a strand should equal the number of base and follow
            the order of bases in that strand. In addition each domain should only contain bases for a single helix.
        """
        self._logger.debug(
            "============================== check domains ============================== ")
        num_failures = 0
        for strand in self.strands:
            ...
            for domain in domain_list:
                ...
                helix_list = [helix]
                self._logger.debug("Domain %d: number of bases: %d" %
                ...
                if len(helix_list) != 1:
                    self._logger.error("The domain %d references more than one helix: %s" % (
            ...
            if (not strand.is_circular):
                for sbase, dbid in zip(strand.tour, domain_base_ids):
                    if sbase.id != dbid:
                        match_failed = True
                        self._logger.error(
                            "The domain base %d does not match the strand base %d." % (dbid, sbase))
                        num_failures += 1
                        break
            ...
            if match_failed:
                ...
                if num_match_failed == 0:
                    self._logger.error(
                        "Check failed: Domain bases match strand bases but are not in the same order.")
            else:
                self._logger.debug(
        ...
        if num_failures == 0:
            ...
            self._logger.error(
                "Domain consistency check: %d domains failed." % num_failures)
    ...
    def _add_domain(self, id, strand, base_list, merge_domains, msg=""):
        """ Create a DnaDomain object from a list of bases.
            ...
            Arguments:
                ...
                merge_domains (bool): If True then for circular strands merge the
                    bases from the start of the strand with those from the end.
        ...
        if merge_domains and strand.is_circular and len(strand.domain_list):
            ...
            if (first_sbase.h == ebase.h) and (abs(first_sbase.p-ebase.p) == 1):
                self._logger.debug(
                    "---------- strand %d: merging domains ---------- " % strand.id)
                self._logger.debug(">>> first domain id %d:  h %d  sb %d  eb %d" % (
                    first_dom.id, first_sbase.h, first_sbase.p, first_ebase.p))
                self._logger.debug(
                    ">>> base list:           sbase %d  ebase %d" % (sbase.p, ebase.p))
        ...
        if not domain_was_merged:
            ...
            self._logger.debug("     vh:  %4d  -  %4d" %
                               (base_list[0].h, base_list[-1].h))
            self._logger.debug("     pos: %4d  -  %4d" %
    ...
    def _set_helix_connectivity(self):
        ...
        for helix1 in self.structure_helices_map.values():
            self._logger.debug(" ----- vhelix num %d -----" %
                               helix1.lattice_num)
            helix_connectivity = []
    ...
    def write(self, file_name, write_json_format):
        ...
        with open(file_name, 'w') as outfile:
            ...
            outfile.write(
                "# bases: id   helix  pos   up   down  across  seq   strand   scaf\n")
            for base in self.base_connectivity:
            ...
            outfile.write(
                "# strands: id  scaf  numBases:[baseIDs]  numDomains:[domainIDs]\n")
            for strand in self.strands:
                outfile.write("strand %4d %2d \n" %
                              (strand.id, strand.is_scaffold))
                outfile.write("%d:%s\n" % (len(strand.tour), str(
                    [base.id for base in strand.tour])))
    ...
    def write_topology(self, file_name, write_json_format):
        ...
        if write_json_format:
            self._logger.info(
                "Writing DNA base connectivity in JSON format to file %s." % file_name)
            base_list = []
            for base in self.base_connectivity:
                base_info = OrderedDict()
                base_info['id'] = base.id
        ...
        file_name = file_name.replace("json", "txt")
        self._logger.info(
            "Writing DNA base connectivity in plain text format to file %s." % file_name)
        with open(file_name, 'w') as outfile:
            outfile.write(
