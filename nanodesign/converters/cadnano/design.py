...
class CadnanoDesign(object):
    ...
    def calculate_possible_crossovers(self):
        ...
        self._logger.debug(">>> lattice type: %s " %
                            ...
                            if index < num_bases:
                                if st == CadnanoStrandType.SCAFFOLD:
                                    ...
                                    vhelix.possible_staple_crossovers.append(
                                        (neighbor, index))
                                self._logger.debug(
                                    "    add i: %4d  j: %4d  index: %4d" % (i, j, index))
...
class CadnanoVirtualHelix(object):
    ...
    def __init__(self, id, num, row, col, insertions, deletions, scaffold_sequence, staple_sequence, modifications, unsupported_data):
        self.id = id
        self.num = num
        self.row = row
        self.col = col
