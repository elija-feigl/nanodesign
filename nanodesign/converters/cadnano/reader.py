...
class CadnanoReader(object):
    ...
    def parse_json_data(self, json_data):
        ...
        if ((num_bases % 21 == 0) and (num_bases % 32) == 0):
            ...
            lattice_type = CadnanoLatticeType.honeycomb
        self._logger.info("Lattice type %s " %
                          CadnanoLatticeType.names[lattice_type])
        ...
        for json_helix in json_data[CadnanoJsonFields.VSTRANDS]:
            ...
            helix = CadnanoVirtualHelix(num_helix, num, row, col, insertions, deletions, scaffold_sequence, staple_sequence)
            self._logger.debug(
            ...
            max_row_json = max(max_row_json, int(
                json_helix[CadnanoJsonFields.ROW])+1)
            max_col_json = max(max_col_json, int(
                json_helix[CadnanoJsonFields.COL])+1)
            num_helix += 1
