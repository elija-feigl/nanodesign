...
class CadnanoConvertDesign(object):
    ...
    def create_structure(self, design, modify=False):
        ...
        """
        self._logger.info(
            "Distance between adjacent helices %g " % self.dna_parameters.helix_distance
        )
        ...
        for bindex, base in self.base_map.items():
            ...
            if print_dna_model_base_map:
                self._logger(
                    "%4d: bindex %12s  base id %4d  up %4s  down %4s  across %4s  h %4d  p %4d  scaf %d"
                    % (
                        n,
                        bindex,
                        base.id,
        ...
        if self._logger.getEffectiveLevel() == logging.DEBUG:
            ...
            for i in range(0, num_bases):
                ...
                across = base.across.id if base.across else -1
                self._logger.debug(
                    "%4d  id %4d  h %4d  p %4d  up %4d  down %4d  across %4d  scaf %d"
        ...
        if modify:
            ...
            if print_base_connectivity:
                self._logger.info(
                    "Size of topology after deletes %d" % len(
                        base_connectivity)
                )
                self._logger.info(
                ...
                for i, base in enumerate(base_connectivity):
                    ...
                    across = base.across.id if base.across else -1
                    self._logger.info(
                        "%4d: base id %4d  up %4s  down %4s  across %4s  h %4d  p %4d  scaf %d"
        ...
        if modify:
            ...
            if print_base_connectivity:
                self._logger.info(
                    "Size of topology after inserts %d" % len(
                        base_connectivity)
                )
                self._logger.info(
    ...
    def _insert_bases(self, helices, base_connectivity):
        ...
        for curr_base in base_inserts.values():
            ...
            if curr_base.up:
                ...
                self._logger.debug(
                    "Current base up   ID %3d  h %3d  p %3d  (%6.2f %6.2f %6.2f) "
                    % (
                        up_base.id,
                        up_base.h,
                        up_base.p,
            ...
            if curr_base.down:
                base_down = curr_base.down
                coords = base_down.coordinates
                self._logger.debug(
                    "Current base down ID %3d  h %3d  p %3d  (%6.2f %6.2f %6.2f) "
                    % (
                        base_down.id,
                        base_down.h,
                        base_down.p,
        ...
        for helix_id in new_bases.keys():
            ...
            self._logger.debug(
                "Helix ID %d  add num bases %d " % (helix_id, len(base_list))
            )
            helix_map[helix_id].insert_bases(base_list)

        self._logger.info("Number of inserted bases %d" % num_insert_bases)

    def _insert_bases_ssDNA(
        self, curr_base, base_id, helix_axis, new_bases, base_connectivity
    ):
        ...
        """
        dist_bp = (
            self.dna_parameters.base_pair_rise
    ...
    def _create_single_helix(self, vhelix, lattice_type):
        ...
        scaffold_bases = []
        staple_bases = []
        for helix_pos, (current_scaffold, current_staple) in enumerate(
            zip(scaffolds, staples)
        ):
            # If the base exists in the scaffold strand.
            if (current_scaffold.initial_strand >= 0) or (
                current_scaffold.final_strand >= 0
            ):
                base = self._add_base(
                    StrandType.SCAFFOLD,
                    current_scaffold,
                ...
                base.num_deletions = vhelix.deletions[helix_pos]
                base.num_insertions = vhelix.insertions[helix_pos]
                scaffold_bases.append(base)

            # if the base exists in the staple strand
            if (current_staple.initial_strand >= 0) or (
                current_staple.final_strand >= 0
            ):
                base = self._add_base(
                    StrandType.STAPLE,
                    current_staple,
    ...
    def _add_base(
        self, base_type, base, paired_base_type, paired_base, helix_pos, helix_num
    ):
    ...
    def _set_possible_crossovers(self, design):
        ...
        for vhelix in design.helices:
            ...
            self._logger.debug(
                "            num scaffold cross-overs: %d " % len(
                    scaffold_crossovers)
            )
    ...
    def set_sequence_from_scaffold(self, dna_structure, modified_structure, sequence):
        ...
        Arguments:
            ...
            sequence (string): The sequence of the scaffold strand
        """
        seq_index = 0
    ...
    def set_sequence(self, dna_structure, modified_structure, sequence):
        ...
        Arguments:
            ...
            sequences list(DnaSequence): A list of DnaSequence objects representing
                the sequences for staple or scaffold strands.
        """
        strands = dna_structure.strands
