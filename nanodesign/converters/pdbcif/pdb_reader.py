...
"""
This module is used to read PDB format files used internally to create atomic
models of DNA nanodesigns.

The PDB files are used as templates to create atomic models for nanodesigns.
Only ATOM records are read.
"""
import logging

from .atomic_types import Molecule, Atom
...
class PdbReader(object):
    ...
    def read(self, file_name):
        ...
        self._logger.info(
            "Read %d chains: %s " % (
                len(self.current_molecule.chains),
                str(list(self.current_molecule.chains))
            )
        )
