...
from .model import VisModel, VisModelRepType
...
class VisCommand(object):
    ...
    def __init__(self, model: VisModel, cmd_file_name, commands):
        """
    ...
    def proc_helix_cmd(self, cmd, tokens):
        ...
        if (show and self.update_menu):
            delay = True
            self.model.menu.update_submenu_selection(
                VisMenuEntity.HELIX, name, rep, delay)
    ...
    def generate_atomic_struct_cmd(self, atom_struct_name, atom_struct_rep, show):
        ...
        if atom_struct_name == VisCommand.NONE:
            cmd = VisCommand.SHOW_ATOMIC_STRUCTURE % (
                'All', atom_struct_rep, 'false')
        elif atom_struct_name == VisCommand.ALL:
            cmd = VisCommand.SHOW_ATOMIC_STRUCTURE % (
                atom_struct_name, atom_struct_rep, 'true')
        else:
            if show:
                show_str = 'true'
            else:
                show_str = 'false'
            cmd = VisCommand.SHOW_ATOMIC_STRUCTURE % (
                atom_struct_name, atom_struct_rep, show_str)
        self.write_cmd(cmd)
        self.execute_cmd(cmd)
    ...
    def proc_model_cmd(self, cmd, tokens):
        ...
        rep = rep.replace("_", " ")
        if rep == VisModelRepType.BOUNDING_BOX:
            self.model.show_bounding_box(show)
        elif rep == VisModelRepType.GEOMETRY:
            self.model.show_structure_geometry(show)
        elif rep == VisModelRepType.HELIX_NUMBERS:
            self.model.show_helix_numbers(show)
        elif rep == VisModelRepType.HELIX_PROJECTION:
    ...
    def proc_strand_cmd(self, cmd, tokens):
        ...
        for token in tokens:
            ...
            if arg_name not in args:
                self._logger.error(
                    "Unknown strand argument \'%s\' " % arg_name)
                return
            if arg_name == 'name':
                names.append(arg_value)
            elif arg_name == 'names':
        ...
        if (show and self.update_menu):
            delay = True
            self.model.menu.update_submenu_selection(
                VisMenuEntity.STRAND, name, rep, delay)
    ...
    def proc_atomic_struct_cmd(self, cmd, tokens):
        ...
        for token in tokens:
            ...
            if arg_name not in args:
                self._logger.error(
                    "Unknown atomic structure argument \'%s\' " % arg_name)
                return
            if arg_name == 'name':
                name = arg_value
            elif arg_name == 'rep':
        ...
        if (show and self.update_menu):
            delay = True
            self.model.menu.update_submenu_selection(
                VisMenuEntity.ATOMIC_STRUCTURE, name, rep, delay)

    def generate_graphics_cmd(self, name, value):
        """ Generate a graphic command. """
        if name == "center":
            cmd = "graphics  center=(%g,%g,%g)" % (
    ...
    def proc_graphics_cmd(self, cmd, tokens):
        ...
        for token in tokens:
            ...
            if arg_name not in args:
                self._logger.error(
                    "Unknown graphics argument \'%s\' " % arg_name)
                return
            if arg_name == 'center':
                # print("proc_graphics_cmd value %s" % arg_value)
                values = arg_value[arg_value.find(
                    "(")+1:arg_value.find(")")].split(",")
