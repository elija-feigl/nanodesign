...
class VisAtomicStructure(object):
    ...
    def show(self, rep, show, display=True):
        ...
        """
        self._logger.debug(
            "Show atomic structure \'%s\'  rep \'%s\' " % (self.name, rep))
    ...
    def print_info(self):
        ...
        self._logger.info("Number of residues %d" %
                          (len(self.molecule.residues)))
    ...
    def create_bonds_rep(self):
        ...
        self._logger.debug("Number of residues %d " %
                           len(self.molecule.residues))
        num_bond_points = 0
        bond_indexes = []
        bond_points = []
        ...
        for i in range(0, num_res):
            ...
            atom = residue_atoms["P"]
            self._get_residue_bond_geometry(
                bond_points, atom.res_name, residue_atoms)
            self._get_residue_plane_geometry(
                plane_counts, plane_points, atom.res_name, residue_atoms)
            if i != num_res-1:
                next_residue_atoms = res_list[i+1]
                self._add_residue_bond(
                    "P", residue_atoms, "O3'", next_residue_atoms, bond_points)
    ...
    def create_check_rep(self):
        ...
        for i in range(0, len(points)-1):
            pt1 = points[i]
            pt2 = points[i+1]
            v = [pt1[j] - pt2[j] for j in range(0, 3)]
    ...
    def _get_residue_plane_geometry(self, plane_counts, plane_points, res_name, residue_atoms):
        ...
        res_name = res_name.lower()
        # Add the sugar ring.
        self._add_planes(residue_atoms, VisDnaPlanes.sugar,
                         plane_counts, plane_points)
        # Add the purine or pyrimidine ring(s).
        if res_name == "da":
            self._add_planes(residue_atoms, VisDnaPlanes.ade,
                             plane_counts, plane_points)
        elif res_name == "dc":
            self._add_planes(residue_atoms, VisDnaPlanes.cyt,
                             plane_counts, plane_points)
        elif res_name == "dg":
            self._add_planes(residue_atoms, VisDnaPlanes.gua,
                             plane_counts, plane_points)
        elif res_name == "dt":
            self._add_planes(residue_atoms, VisDnaPlanes.thy,
                             plane_counts, plane_points)
    ...
    def _add_bonds(self, residue_atoms, bonds, points):
        ...
        for bond in bonds:
            ...
            points.append(
                [s*atom1.coords[0], s*atom1.coords[1], s*atom1.coords[2]])
            points.append(
                [s*atom2.coords[0], s*atom2.coords[1], s*atom2.coords[2]])
