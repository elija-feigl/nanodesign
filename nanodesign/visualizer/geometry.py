...
class VisGeometryBox(VisGeometry):
    ...
    def _create_geometry(self, xmin, xmax, ymin, ymax, zmin, zmax):
        ...
        # Minimum Z face.
        z = zmin
        vertices[n, :] = [xmin, ymin, z]
        n += 1
        vertices[n, :] = [xmax, ymin, z]
        n += 1
        vertices[n, :] = [xmax, ymin, z]
        n += 1
        vertices[n, :] = [xmax, ymax, z]
        n += 1
        vertices[n, :] = [xmax, ymax, z]
        n += 1
        vertices[n, :] = [xmin, ymax, z]
        n += 1
        vertices[n, :] = [xmin, ymax, z]
        n += 1
        vertices[n, :] = [xmin, ymin, z]
        ...
        # Bottom Y face.
        vertices[n, :] = [xmin, ymin, zmin]
        n += 1
        vertices[n, :] = [xmin, ymin, zmax]
        n += 1
        vertices[n, :] = [xmax, ymin, zmin]
        n += 1
        vertices[n, :] = [xmax, ymin, zmax]
        n += 1

        # Top Y face.
        vertices[n, :] = [xmax, ymax, zmin]
        n += 1
        vertices[n, :] = [xmax, ymax, zmax]
        n += 1
        vertices[n, :] = [xmin, ymax, zmin]
        n += 1
        vertices[n, :] = [xmin, ymax, zmax]
        n += 1
...
class VisGeometryLines(VisGeometry):
    ...
    def _create_geometry(self, points, arrows):
        ...
        if arrows:
            ...
            self.arrow_vertices = np.zeros(
                (self.num_arrow_vertices, 3), dtype=float)
            n = 0
    ...
    def render(self):
        ...
        if self.num_arrow_vertices:
            glBegin(GL_LINES)
            for i in range(0, self.num_arrow_vertices):
    ...
    def _create_geometry(self, points, colors, show_vertices, show_arrows):
        ...
        if show_arrows:
            ...
            self.arrow_vertices = np.zeros(
                (self.num_arrow_vertices, 3), dtype=float)
            if self.colors:
                self.arrow_colors = np.zeros(
                    (self.num_arrow_vertices, 4), dtype=float)
            n = 0
            for i in range(0, len(self.bend_points)+offset):
    ...
    def render(self):
        ...
        if self.start_marker:
            if not self.start_sphere:
                ...
                self.start_sphere = VisGeometrySphere(
                    name, self.vertices[0], self.start_marker_radius)
...
class VisGeometryCylinder(VisGeometry):
    ...
    def render(self):
        ...
        for i in range(0, num_tri-(cap1+cap2)*num_sides):
            glBegin(GL_POLYGON)
            for j in range(0, n):
                k = conn[conn_count]
                nx = norms[k][0]
    ...
    def _create_geometry(self, origins, directions, scale):
        ...
        for i in range(0, num_axes):
            for j in range(0, 3):
                ...
                self.vertices[n+1, j] = origins[i, j] + \
                    scale*directions[j, 0, i]
                self.vertices[n+2, j] = origins[i, j]
                self.vertices[n+3, j] = origins[i, j] + \
                    scale*directions[j, 1, i]
                self.vertices[n+4, j] = origins[i, j]
                self.vertices[n+5, j] = origins[i, j] + \
                    scale*directions[j, 2, i]
                # Arrow head.
                self.vertices[n+6, j] = origins[i, j] + \
                    scale*directions[j, 2, i]
                self.vertices[n+7, j] = origins[i, j] + hs * \
                    directions[j, 2, i] + asc*directions[j, 1, i]
                self.vertices[n+8, j] = origins[i, j] + \
                    scale*directions[j, 2, i]
                self.vertices[n+9, j] = origins[i, j] + hs * \
                    directions[j, 2, i] - asc*directions[j, 1, i]
                self.vertices[n+10, j] = origins[i, j] + \
                    scale*directions[j, 2, i]
                self.vertices[n+11, j] = origins[i, j] + hs * \
                    directions[j, 2, i] + asc*directions[j, 0, i]
                self.vertices[n+12, j] = origins[i, j] + \
                    scale*directions[j, 2, i]
        ...
        for i, n in enumerate(self.counts):
            ...
            for j in range(0, n):
                if j == n-1:
                    ...
                    k = j+1
                nx += (verts[j+pindex][1] - verts[k+pindex][1]) * \
                    (verts[j+pindex][2] + verts[k+pindex][2])
                ny += (verts[j+pindex][2] - verts[k+pindex][2]) * \
        ...
        for i, n in enumerate(self.counts):
            ...
            for j in range(0, n):
                if j == n-1:
                    ...
                    k = j+1
                poly_pts = [self.vertices[j+pindex],
                            self.vertices[k+pindex], self.centers[i]]
                ipt = comp_poly_line_intersect(
                    normal, poly_pts, point1, point2)
                if ipt:
                    intersect_indexes.append(i)
                    intersect_points.append(ipt)
            # __for j in range(0,n)
...
class VisGeometryNumber(VisGeometry):
    ...
    def _generate_number(self):
        ...
        offset = [origin[j] - nd*width*u[j] for j in range(0, 3)]
        points = []
        n = 0
        for digit in self.number:
            ...
            for i in range(0, len(pattern)-1):
                ...
                if not disp1 or not disp2:
                    continue
                point1 = [offset[j] + width*disp1[0]*u[j] +
                          width*disp1[1]*v[j] for j in range(0, 3)]
                point2 = [offset[j] + width*disp2[0]*u[j] +
                          width*disp2[1]*v[j] for j in range(0, 3)]
...
def generate_arrow_geometry(point1, point2, num_arrow_pts=4):
    ...
    verts = []
    for i in range(0, num_arrow_pts):
        ct = r*cos(t)
        st = r*sin(t)
        verts.append(point2)
