...
class VisHelix(object):
    ...
    def print_info(self):
        ...
        self._logger.info("Polarity %s " %
                          (str(self.vhelix.scaffold_polarity)))
        self._logger.info("Number of domains %d " %
                          (len(self.vhelix.get_domain_ids())))
    ...
    def create_crossovers_rep(self):
        ...
        self._logger.debug(
            "Create crossover rep for helix num %d " % self.vhelix.lattice_num)
        ...
        for connection in helix_connectivity:
            ...
            self._logger.debug("    Direction (%g %g %g) " %
                               (dir[0], dir[1], dir[2]))
            self._logger.debug("    Number of crossovers %d " %
                               len(crossovers))
            self._logger.debug(
                "    Number of helix_axis_coords %d " % len(helix_axis_coords))

            for i in range(0, len(crossovers), 1):
    ...
    def create_maximal_crossovers_rep(self):
        ...
        self._logger.debug(
            "Number of possible staple crossovers: %d " % len(staple_crossovers))
        self._logger.debug(
            "Number of possible scaffold crossovers: %d " % len(scaffold_crossovers))
    ...
    def select_coords(self, geom, index):
        ...
        if "Scaffold" in geom.name:
            ...
            strand_type = "staple"
        self._logger.info(
            "Selected Helix %s coordinates. Base number %d" % (self.name, index+1))
    ...
    def create_temperature_rep(self):
        ...
        # Vector pointing along the helix axis.
        axis = self.vhelix.helix_axis_frames[:, 2, 0]
        ...
        for i, key in enumerate(sorted(domain_map, reverse=sort_reversed)):
            ...
            point1 = [point1[j] + s*base_pair_rise*axis[j]
                      for j in range(0, 3)]
            point2 = [point2[j] - s*base_pair_rise*axis[j]
                      for j in range(0, 3)]
            temp = domain.melting_temperature()
            self._logger.debug("Domain ID %d  #bases %d  temp %g" %
                               (id, len(domain.base_list), temp))
            if temp == -500.0:
                color = [0.5, 0.5, 0.5]
            else:
            ...
            if i == 0:
                ...
                capped = (False, False)
            geom = VisGeometryCylinder(
                name, radius, point1, point2, capped=capped)
            geom.color = color
    ...
    def select_domains_temperature(self, geom, index):
        ...
        self._logger.info("Domain ID %d  Number of bases %d  Start pos %d  End pos %d" % (domain_id, num_bases,
                                                                                          start_base.p, end_base.p))
        self._logger.info("Domain melting temperature is %g" %
                          (domain.melting_temperature()))
        self._logger.info("Domain is part of strand %s" % (strand_name))
        self.print_info()
    ...
    def create_inserts_deletes_rep(self):
        ...
        inserts = [i for i, base in enumerate(
            self.vhelix.staple_base_list) if base.num_insertions != 0]
        ...
        if num_inserts != 0:
            ...
            inserts_geom = VisGeometrySymbols(
                name, symbol, insert_origins, insert_directions, scale)
        ...
        deletes = [i for i, base in enumerate(
            self.vhelix.staple_base_list) if base.num_deletions != 0]
        ...
        if num_deletes != 0:
            ...
            deletes_geom = VisGeometrySymbols(
                name, symbol, delete_origins, delete_directions, scale)
    ...
    def select_strand(self, geom, index):
        ...
        self._logger.info("Selected Helix %s Strand %s" %
                          (self.name, strand_name))
        self._logger.info("Location in strand path %d  Vhelix %d  Position %d  " % (
